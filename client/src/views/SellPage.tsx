    ...
    <SwapShell>
      ...
      <Card>
        <div className="mb-5 flex items-center justify-between">
          <p className="text-sm font-medium text-white/60">You're selling</p>
        ...
        <div className="flex justify-center gap-2.5">
          {["25%", "50%", "75%", "Max"].map(p => (
            <Pill key={p} label={p} active={amount === p} onClick={() => setAmount(p)} />
          ))}
        </div>
      </Card>
      ...
      {!address
        ...
        : <ActionBtn>Sell {token.symbol}</ActionBtn>
      }
    </SwapShell>
  );
}
